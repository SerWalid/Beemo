...
from .utils import login_required, pin_required, logged_in_restricted, generate_time_array, calculate_age, time_difference_from_today  # Import the decorator
...
def parent():
    ...
    return render_template('dashboardHome.html',chats= chat_list, child_name=child_name, parent_name=parent_name, goals=goals, beemo_daily_usage_timeline=beemo_daily_usage_timeline, dashboard_stats=dashboard_stats, user=user)
...
def settings():
    ...
    if settings is None:
        ...
        settings = Settings(user_id=user_id, daily_usage_limit=0, goals=goals_string, notifications_preferences=notifications_preferences_string)        
    ...
    return render_template('settings.html',chats= chat_list, banned_topics=selected_topics, user=user, time_array=time_array, sleep_time_start=sleep_time_start, sleep_time_end=sleep_time_end, daily_usage_limit=daily_usage_limit, word_goal=word_goal, reading_time_goal=reading_time_goal, sessions_per_week_goal=sessions_per_week_goal, alert_topics=alert_topics, high_emotional_engagement=high_emotional_engagement, repetitive_questions=repetitive_questions, agitated_behavior=agitated_behavior)
...
def get_chat_by_id(id):
    ...
    return render_template('chat-history.html', interactions = chat_data, user=user, chats= chat_list, chat_date=chat_date, number_of_interactions=number_of_interactions)